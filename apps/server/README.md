...
- Bucket/Database: `infra/bucket` 与 `infra/database` 的 index 只保留共享契约和 runtime-neutral helper；process/isolate 实现由 runtime capability 显式注册，process 侧缓存资源，isolate 侧按 request binding 创建并保留 no-op disposer。
...
| [product-export.md](./docs/reference/product-export.md) | 产品 CSV 导出 job、模板查询、分页列表、下载、part 聚合与数据库 store 边界                    |
...
| Runtime capability | database、bucket、queue、scheduler、file 能力 | runtime entry 显式注册具体 adapter，`disposeRuntimeCapabilities()` 按注册 disposer 释放 |
